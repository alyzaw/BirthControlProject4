...
#     if param == '':
#         table_df = pd.read_sql("SELECT Method, Birth_Control, Vader_Scale, `Use`, Star_Rating, Date, Review  FROM table_db ORDER BY Method",conn)
#     else:    
#         table_df = pd.read_sql(f"SELECT Method, Birth_Control, Vader_Scale, `Use`, Star_Rating, Date, Review FROM table_db WHERE Method = '{param}' ORDER BY Method",conn)
